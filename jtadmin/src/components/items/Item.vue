...
<!-- 添加修改分类对话框 -->
    <el-dialog title="修改商品列表" :visible.sync="updateItemDialogVisible" width="50%" @close="closeUpdateDialog">
      <!-- 定义分类表单 -->
      <el-form :model="updateItemForm" :rules="rules" ref="upDateItemForm" label-width="100px">
        <el-form-item label="商品标题:" prop="title">
          <el-input v-model="updateItemForm.title"></el-input>
        </el-form-item>
        <el-form-item label="商品卖点:" prop="sellPoint">
          <el-input v-model="updateItemForm.sellPoint"></el-input>
        </el-form-item>
        <el-form-item label="商品价格:" prop="price">
          <el-input v-model="updateItemForm.price"></el-input>
        </el-form-item>
        <el-form-item label="商品数量:" prop="num">
          <el-input v-model="updateItemForm.num"></el-input>
        </el-form-item>
      </el-form>
  ...
  export default {
    ...
    methods: {
      ...
      async updateItemBtn(item){
        ...
        this.updateItemForm = result.data

        // this.item.price = (this.item.price / 100).toFixed(2)
      },
      closeUpdateDialog(){
        //重置表格数据
        this.$refs.upDateItemForm.resetFields()
      },
      updateItem() {
        this.$refs.upDateItemForm.validate(async valid => {
         if(!valid)  return this.$message.error("列表验证没有通过")
        //修改商品分类信息
        const {
          data: result
        } = await this.$http.put('/item/updateItem', this.updateItemForm)
        if (result.status !== 200) return this.$message.error("更新商品列表失败")
